...
public final class DataManagementService {
    ...
    public File exportCsv(long ledgerId) throws IOException {
        ...
        StringBuilder csv = new StringBuilder(
                "\uFEFF日期,类型,分类,金额,币种,账户,转入账户,记账人,标签,报销,计入预算,备注\n");
        ...
        try (Cursor cursor = helper.getReadableDatabase().rawQuery(sql,
            ...
            while (cursor.moveToNext()) {
                String[] fields = {
                        V13Ui.dateTime(cursor.getLong(0)),
                        V13Ui.typeName(cursor.getString(1)),
                        cursor.getString(2),
                        java.math.BigDecimal.valueOf(cursor.getLong(3), 2).toPlainString(),
                        cursor.getString(4), cursor.getString(5), cursor.getString(6),
                        cursor.getString(7), cursor.getString(8),
                        cursor.getInt(9) == 1 ? "是" : "否",
                        cursor.getInt(10) == 1 ? "是" : "否",
                        cursor.getString(11)
                };
    ...
    public void restoreJsonBackup(File file) throws IOException {
        ...
        try {
            ...
            for (String table : INSERT_ORDER) {
                importTable(db, table, tables.getJSONArray(table));
            }
    ...
    public List<File> listBackups() {
        File[] files = directory("backups").listFiles(
                (dir, name) -> name.endsWith(".json"));
        List<File> result = files == null
                ? new ArrayList<>() : new ArrayList<>(Arrays.asList(files));
    ...
    public static String backupFileName(long time, boolean safety) {
        return "ledgerbook-" + (safety ? "safety-" : "backup-")
                + timestamp(time) + ".json";
    }

    private static String timestamp(long time) {
        return new SimpleDateFormat(
                "yyyyMMdd-HHmmss", Locale.ROOT).format(new Date(time));
    }

    private File directory(String child) {
        File documents = context.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS);
        File base = documents == null
                ? new File(context.getFilesDir(), "documents") : documents;
                ...
                for (int column = 0; column < cursor.getColumnCount(); column++) {
                    ...
                    switch (cursor.getType(column)) {
                        ...
                        case Cursor.FIELD_TYPE_BLOB:
                            row.put(name, android.util.Base64.encodeToString(
                                    cursor.getBlob(column), android.util.Base64.NO_WRAP));
                            break;
                        default:
    ...
    private static String read(File file) throws IOException {
        ...
        try (FileInputStream input = new FileInputStream(file)) {
            ...
            if (offset != buffer.length) {
                throw new IOException("备份文件读取不完整");
            }
        }
